...
calculate_flux <- function(start_date = NULL,
  ...
  today <- Sys.time()
  maint_log <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/199z0PPPDuapBQGShuTf5_6kFZ4U0mfkD4DeBuIPWKdE/edit?gid=0#gid=0",
                                         col_types = "c") %>%
  ...
  for(i in 1:nrow(maint_log)){
    data_numeric <- data_numeric %>%
      mutate(Flag = ifelse(TIMESTAMP <= maint_log$End_time[i] & 
                           ...
                           maint_log$Flag[i],
                           Flag),
             across(c("CH4d_ppm", "CO2d_ppm", "N2Od_ppb"), 
  ...
  if(!reprocess){
    ...
    old_slopes <- read_csv(here::here("processed_data","L0.csv"), 
                           col_types = "nnDcccnnnnnnnnnnnnnnnnnnnnnncccc",
                           show_col_types = F) %>%
      mutate(TIMESTAMP = as_datetime(TIMESTAMP, tz = "EST"),
