...
slopes_recent <- slopes_l0 %>%
  filter(as.Date(TIMESTAMP, tz = "EST") >= Sys.Date()-1,
         #hour(TIMESTAMP) >= 16,
         #as.Date(TIMESTAMP) <= today,
...
p1 <- slopes_recent %>%
  ...
  ggh4x::facet_grid2(gas~type, scales = "free_y") +
...
p <- slopes_recent %>%
  filter(chamber %in% c(10, 20, 40, 70, 80, 90, 130, 150, 160, 180, 190, 220)) %>%
  mutate(chamber = as.factor(chamber),
         Hour = hour(TIMESTAMP)) %>%
...
p1 <- slopes_recent %>%
  mutate(chamber = as.factor(chamber),
         bare = ifelse(chamber %in% c(110, 120, 230, 240),
                       "air",
                       "chamber")) %>%
  ggplot(aes(x = TIMESTAMP, y = slope, color = chamber)) +
  geom_hline(yintercept = 0, color = "grey70") +
  ...
  geom_line() +
  #geom_vline(xintercept = as_datetime("2025-05-07 14:00:00"), linetype = "dashed", color = "grey70") +
  ggh4x::facet_grid2(gas~bare, scales = "free_y") +
  #scale_color_manual(values = color.gradient)+
  scale_x_datetime(date_breaks = "2 day", date_labels = "%b %d") +
  theme_bw()+
  theme(legend.key.spacing.y = unit(0, "cm"),
